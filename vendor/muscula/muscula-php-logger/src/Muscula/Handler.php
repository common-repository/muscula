...
/**
 * Overrides PHP errors and exceptions handlers, so all errors, exceptions and debug messages will be sent to Muscula client
...
class Handler {
    ...
    public static function setLogId(string $string)
    {
	 ...
	 * @throws \Exception
	 */
	public function start($logId) {
	    $this->connector->logId = $logId;
		if($this->isStarted) {
	...
	/**
	 * Check if Muscula handler is started
	 * @return bool
	 */
	...
	/**
	 * Override PHP exceptions handler to Muscula handler
	 */
	 ...
	 * But, anyway, it's strongly recommended to configure ignore some errors type in Muscula extension options
	...
	/**
	 * Override PHP errors handler to Muscula handler
	 */
